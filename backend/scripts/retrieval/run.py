...
from src.retrieval.indexer.milvus.build_pipeline import BuildMilvusIndexPipeline
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default="configs/indexing.yaml",
        help="Path to retrieval config YAML.",
    )
    parser.add_argument(
        "--task",
        choices=["build-index", "build-milvus-index", "build-vector-cache"],
        default="build-index",
    )
    parser.add_argument(
        "--output", default=None, help="Optional override output path for vector cache."
    )
    parser.add_argument(
        "--dtype",
        default=None,
        help="Optional override dtype for vector cache (float16 or float32).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
        default=50_000,
        help="Batch size for vector cache creation.",
    )
    parser.add_argument(
        "--no-normalize",
        action="store_true",
        help="Do not normalize reconstructed vectors when building cache.",
    )
...
def main() -> None:
    ...
    if args.task == "build-milvus-index":
        pipeline = BuildMilvusIndexPipeline(cfg)
        pipeline.run()
        return
    ...
    if "semantic" in cfg and isinstance(cfg["semantic"], dict):
        models = [
            m for m in cfg["semantic"].get("models", []) if m.get("enabled", True)
        ]
        if not models:
    ...
    index_path = (
        raw_index_path if raw_index_path.is_absolute() else backend_dir / raw_index_path
    )
    output_path = (
        Path(args.output)
        if args.output
        ...
        / Path(
            faiss_cfg.get("vector_cache_path")
            or "data/database/faiss_hnsw_clip_vitl16_siglip_256/vectors_fp16.npy"
        )
    )
