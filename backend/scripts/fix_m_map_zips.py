...
TEMP_DIR = Path("D:/HCMAIC/event-retrieval-system/data/_tmp_fix_m_maps")

# CHỈ THAO TÁC DUY NHẤT NHÓM M (M01 -> M09), TUYỆT ĐỐI KHÔNG CHẠM NHÓM S HOẶC KHÁC
TARGET_ZIPS = [f"M{i:02d}.zip" for i in range(1, 10)]
...
def fix_single_map_zip(zip_name: str) -> bool:
    ...
    # 2. Bóc tách và đưa toàn bộ file .csv ra root của zip (loại bỏ folder Mxx/ ở giữa)
    ...
    with zipfile.ZipFile(downloaded_zip, "r") as zf_in:
        ...
        with zipfile.ZipFile(fixed_zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf_out:
            for info in csv_entries:
                # Lấy tên file gốc (ví dụ: M01_V001.csv) để đặt trực tiếp ở root của zip
    ...
    log(f"Chuẩn hóa {zip_name}: {csv_count} files CSV -> {new_size_kb:.1f} KB (Flat CSVs, không còn folder con)")
...
def main():
    ...
    log("BẮT ĐẦU CHUẨN HÓA CÁC FILE MAP ZIP NHÓM M (M01 -> M09)")
    log("=" * 60)
    
    for zip_name in TARGET_ZIPS:
        ok = fix_single_map_zip(zip_name)
        if ok:
            success += 1
        else:
            failed += 1
