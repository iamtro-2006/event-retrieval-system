...
from src.core.clustering import select_cluster_keyframes
...
def save_keyframe_images(indexes: list[int], video_path: Path, output_dir: Path, image_quality: int) -> None:
    ...
    for keyframe_id, frame_idx in enumerate(indexes):
        ...
        if frame is None:
            print(
                f"[SKIP] keyframe_id={keyframe_id} frame_idx={frame_idx} read_frame=None",
                flush=True,
            )
            continue
        ...
        print(
            f"[SAVE_FRAME] keyframe_id={keyframe_id} "
        ...
        print(
            f"[SAVE_DONE] keyframe_id={keyframe_id} "
            f"frame_idx={frame_idx} "
            f"ok={ok} "
            f"path={out_path}",
            flush=True,
        ...
        if keyframe_id < 5:
            debug_path = output_dir / f"debug_{keyframe_id:06d}_frame_{frame_idx}.png"
            cv2.imwrite(str(debug_path), frame)
            print(f"[DEBUG_PNG] {debug_path}", flush=True)