...
from src.models.dataset import build_loader
from src.models.qwen_engine import extract_text_lines, load_qwen_model


class ExtractOCRPipeline:
    """Runs OCR extraction with one of two engines (extraction.engine):

        "qwen"           -> full-frame OCR via the Qwen3-VL-4B-Instruct-Uncensored
                             GGUF quant (mradermacher/Qwen3-VL-4B-Instruct-Uncensored-GGUF),
                             run locally through llama-cpp-python (model + mmproj
                             vision projector). Default. Falls back to
                             "paddle_vietocr" at runtime if the Qwen model fails
                             to load.
    ...
    def __init__(self, cfg: dict, logger: logging.Logger | None = None):
        ...
        if self.engine == "qwen":
            ...
            try:
                self.qwen_model, self.qwen_processor = load_qwen_model(
                    ...
                    filename=qwen_cfg.get("filename", "Qwen3-VL-4B-Instruct-Uncensored.Q4_K_M.gguf"),
                    mmproj_filename=qwen_cfg.get(
                        "mmproj_filename", "Qwen3-VL-4B-Instruct-Uncensored.mmproj-f16.gguf"
                    ),
        ...
        if self.engine == "paddle_vietocr":
            from src.models.engine import load_ocr_model
            ...
            if self.recognizer == "vietocr":
                from src.models.vietocr_engine import load_vietocr_model
    ...
    def _recognize(self, img, boxes: list[list[float]], paddle_texts: list[str]) -> list[str]:
        ...
        from src.models.vietocr_engine import crop_box, recognize_crops
            ...
            if self.engine == "qwen":
                for path, img in zip(paths, imgs):
                    ...
                    texts = extract_text_lines(
                        ...
                        temperature=float(self.qwen_cfg.get("temperature", 0.0)),
                        logger=self.logger,
                    )
